...
function MammozUI:CreateWindow(config)
	...
	local supportBadge = make("Frame", {
		Name = "SupportBadge",
		-- Fixed navy (not T.Element) so the logo badge stays dark on the
		-- sidebar even though card elements use the light theme.
		BackgroundColor3 = Color3.fromRGB(15, 45, 80),
		BorderSizePixel = 0,
...
function Window:Notify(title, body, kind)
	...
	local toast = make("Frame", {
		...
		-- Toasts stay dark navy regardless of the light card theme so the
		-- white toast text keeps its contrast.
		BackgroundColor3 = Color3.fromRGB(11, 31, 56),
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	corner(toast, 7)
	local toastLine = stroke(toast, Color3.fromRGB(30, 81, 132), 1)
